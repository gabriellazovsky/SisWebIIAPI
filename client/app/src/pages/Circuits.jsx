...
function FlyTo({ circuit }) {
  ...
  useEffect(() => {
    if (circuit?.lat && circuit?.lng) {
      const point = map.project([circuit.lat, circuit.lng], 11); point.x += 230; const shifted = map.unproject(point, 11); map.flyTo(shifted, 11, { duration: 1.4 });
    }
  }, [circuit, map]);
  return null;
}
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ paddingRight: 32 }}>
          ...
          <div style={{ fontFamily: "var(--fd)", fontSize: 26, fontWeight: 900, textTransform: "uppercase", letterSpacing: 0.5, lineHeight: 1.1, color: "var(--text)" }}>
            {circuit.name}
          </div>
          <div style={{ fontSize: 13, color: "var(--muted2)", marginTop: 6, display: "flex", alignItems: "center", gap: 8 }}>
            <span>📍</span>
        ...
        <button
          ...
          style={{
            ...
            cursor: "pointer", color: "var(--muted2)", fontSize: 14,
            transition: "all 0.15s",
          }}
        >✕</button>
      </div>

      <div style={{ padding: "8px 14px", borderBottom: "1px solid var(--border)", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 5 }}>
        {[
          ...
          <div key={item.label} style={{
            background: "var(--dark)", borderRadius: 5, padding: "6px 10px",
      ...
      <div style={{ padding: "10px 18px 6px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ fontFamily: "var(--fd)", fontSize: 12, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: "var(--muted2)" }}>
          Races held here
        </div>
        {!loadingRaces && (
          <span className="badge badge-red">{races.length}</span>
        )}
      </div>
...
export default function CircuitsPage() {
  ...
  useEffect(() => {
    fetch(`${API}/circuits`)
      .then(r => r.json())
      .then(d => { const list = Array.isArray(d) ? d : []; setCircuits(list); setFiltered(list); setLoading(false); })
    ...
    <div style={{ height: "calc(100vh - 52px)", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontFamily: "var(--fd)", fontSize: 28, fontWeight: 900, textTransform: "uppercase", letterSpacing: 1, lineHeight: 1 }}>
            Circuits
          </div>
          <div style={{ fontSize: 12, color: "var(--muted)", marginTop: 3 }}>
            {circuits.length} circuits worldwide — click any to explore
          </div>
        </div>
              ...
              {filtered.map(c => c.lat && c.lng ? (
                ...
                >
                  <Popup>
                    ...
                    <span style={{ fontSize: 12, color: "#555" }}>{c.location}, {c.country}</span>
                  </Popup>
                </Marker>
              ) : null)}
