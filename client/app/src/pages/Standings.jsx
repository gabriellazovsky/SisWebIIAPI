...
export default function StandingsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const dMap = {};
      allDrivers.forEach(d => { dMap[d.driverId] = `${d.forename} ${d.surname}`; });
      setDriverMap(dMap);

      const constrsData = await constrsRes.json();
      const allConstrs = Array.isArray(constrsData) ? constrsData : [];
      const cMap = {};
      allConstrs.forEach(c => { cMap[c.constructorId] = c.name; });
      setConstrMap(cMap);
          ...
          <div style={{ background: "var(--surface)", border: "2px solid var(--red)", borderRadius: 8, padding: 20, display: "flex", alignItems: "center", gap: 16 }}>
            ...
            <div>
              ...
              <div style={{ fontFamily: "var(--fd)", fontSize: 24, fontWeight: 900, textTransform: "uppercase" }}>
                {driverMap[champion.driverId] || `Driver ${champion.driverId}`}
              </div>
          ...
          {constrChampion && (
            <div style={{ background: "var(--surface)", border: "2px solid var(--teal)", borderRadius: 8, padding: 20, display: "flex", alignItems: "center", gap: 16 }}>
              ...
              <div>
                ...
                <div style={{ fontFamily: "var(--fd)", fontSize: 24, fontWeight: 900, textTransform: "uppercase" }}>
                  {constrMap[constrChampion.constructorId] || `Constructor ${constrChampion.constructorId}`}
                </div>
                <div style={{ display: "flex", gap: 12, marginTop: 6 }}>
                ...
                <tbody>
                  {drivers.map(s => (
                    <tr key={s._id || s.driverStandingsId} style={{ background: s.position === 1 ? "rgba(225,6,0,0.04)" : undefined }}>
                      ...
                      <td>
                        <div className="dcell">
                          <div className="avatar" style={{ background: tc(s.driverId), width: 28, height: 28, fontSize: 10 }}>
                            {ini(...(driverMap[s.driverId]||"? ?").split(" "))}
                          </div>
                          <span style={{ fontWeight: 600 }}>{driverMap[s.driverId] || `Driver ${s.driverId}`}</span>
                        </div>
                      </td>
                ...
                <tbody>
                  {constructors.map(s => (
                    <tr key={s._id || s.constructorStandingsId} style={{ background: s.position === 1 ? "rgba(0,210,190,0.04)" : undefined }}>
                      ...
                      <td>
                        <div className="dcell">
                          <div style={{ width: 28, height: 28, borderRadius: 4, background: tc(s.constructorId), display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--fd)", fontSize: 12, fontWeight: 700, color: "#fff" }}>
                            {(constrMap[s.constructorId]||"?")[0]}
                          </div>
                          <span style={{ fontWeight: 600 }}>{constrMap[s.constructorId] || `Constructor ${s.constructorId}`}</span>
                        </div>
                      </td>
                      <td><span className="pts">{s.points}</span></td>
                      <td><span style={{ color: "var(--teal)" }}>{s.wins}</span></td>
